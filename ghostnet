...
class Faded:
	...
	def kill_process():
		...
		os.popen('killall -q -9 chromium brave chrome tlauncher steam vivaldi-stable waterfox firefox > /dev/null 2>&1')
		log('dead',err=True,end=True)
	...
	def clear_caches():
		...
		os.popen('bleachbit -c adobe_reader.cache elinks.history \
			emesene.cache epiphany.cache flash.cache flash.cookies google_chrome.cache google_chrome.history \
			links2.history opera.cache opera.search_history opera.url_history system.cache system.tmp 2>&1 /dev/null')
